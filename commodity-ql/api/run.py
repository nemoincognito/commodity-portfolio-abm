...
# ─── Data classes (plain dicts from JSON) ───
...
class Supplier:
    def __init__(self, d):
        self.name = d['name']
        self.max_capacity = d['max_capacity']
        self.init_capacity = d['init_capacity']
        self.fixed_cost = d['fixed_cost']
        self.variable_cost = d['variable_cost']
        self.p_d = d['p_d']
        self.p_r = d['p_r']
        self.capacity_adjust_rate = d.get('capacity_adjust_rate', 0.08)

class Country:
    def __init__(self, d):
        self.name = d['name']
        self.demand = d['demand']
        self.elasticity = d['elasticity']
        self.dom_variable_cost = d['dom_variable_cost']
        self.dom_fixed_cost = d['dom_fixed_cost']
        self.dom_max_capacity = d['dom_max_capacity']
        self.dom_init_capacity = d['dom_init_capacity']
        self.max_dom = d['max_dom']
        self.stor_max = d.get('stor_max', 0)
        self.stor_cost = d.get('stor_cost', 0)
        self.stor_init = d.get('stor_init', 0)
...
class Env:
    ...
    def step(self, actions):
        ...
        for ci in range(nc):
            ...
            # Pass dicts for get_alloc
            sup_dicts = [{'p_d': s.p_d, 'variable_cost': s.variable_cost} for s in sups]
            allocs.append(get_alloc(pi, sup_dicts, ctrs[ci].max_dom)); stor_dec.append(STOR_ACTS[si])
...
def run_model(sups, ctrs, base_spot, train_eps, eval_sims, ep_len):
    ...
    def evl(act_fn):
        ...
        N = eval_sims * ep_len
        ag = lambda arrs: {'m': [round(float(np.mean(a)), 1) for a in arrs], 'h': [round(float(np.percentile(a, 95)), 1) for a in arrs]}
        fs = [v for a in spt for v in a]; sm = np.mean(fs)
        res = {'spot': ag(spt), 'spotS': {'mean': round(float(sm), 2), 'std': round(float(np.std(fs)), 2), 'p95': round(float(np.percentile(fs, 95)), 2)},
               'sup_cap': ag(sct), 'countries': {}}

        for ci, c in enumerate(ctrs):
            cn = c.name; fc = [v for a in ct[cn] for v in a]; mc = np.mean(fc)
            svd = {}; tot = 0
            for si in range(ns): v = sv[cn][si] / N; svd[sups[si].name] = round(float(v), 2); tot += v
            svd['spot'] = round(float(spv[cn] / N), 2); tot += svd['spot']
            svd['domestic'] = round(float(dv[cn] / N), 2); tot += svd['domestic']
            svd['storage_draw'] = round(float(drv[cn] / N), 2); tot += svd['storage_draw']
            mx = {k: round(float(v / max(tot, 1e-9)), 4) for k, v in svd.items()}
            ta = N; pc = np.zeros(len(PROF_NAMES)); sc = np.zeros(3)
            for a in range(N_ACT): pi, si = dec_act(a); pc[pi] += ac[cn][a]; sc[si] += ac[cn][a]
            af = {'allocation': {PROF_NAMES[i]: round(float(pc[i] / ta), 3) for i in range(len(PROF_NAMES))},
                  'storage': {STOR_ACTS[i]: round(float(sc[i] / ta), 3) for i in range(3)}}
            sup_dicts = [{'p_d': s.p_d, 'variable_cost': s.variable_cost} for s in sups]
            bl = np.zeros(ns + 1)
            for a in range(N_ACT): pi, _ = dec_act(a); bl += get_alloc(pi, sup_dicts, c.max_dom) * ac[cn][a]
            bl /= ta
            bw = {sups[si].name: round(float(bl[si]), 4) for si in range(ns)}
            bw['spot'] = round(float(bl[ns]), 4); bw['domestic'] = round(float(max(0, 1 - bl.sum())), 4)
            cbd = {k: round(float(v / N / max(c.demand, 1)), 2) for k, v in cb[cn].items()}
            res['countries'][cn] = {
                'cost': ag(ct[cn]), 'stor': ag(st[cn]), 'dom_cap_ts': ag(dct[cn]),
                's': {'avg_mean': round(float(mc), 2), 'avg_std': round(float(np.std(fc)), 2), 'demand_met': round(float(tot / c.demand * 100), 2)},
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            sups = [Supplier(s) for s in req['suppliers']]
            ctrs = [Country(c) for c in req['countries']]
            base_spot = req.get('base_spot', 55.0)
            train_episodes = min(req.get('train_episodes', 2000), 5000)
            eval_sims = min(req.get('eval_sims', 300), 1000)
            ep_length = min(req.get('ep_length', 36), 60)

            t0 = time.time()
            r = run_model(sups, ctrs, base_spot, train_episodes, eval_sims, ep_length)
            r['meta'] = {
