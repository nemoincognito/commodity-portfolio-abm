...
- **Lags between decisions and outcomes are next period:** This could be true for commodities that are inherently short cycle like wheat or other grasses and maybe US fracking but is wholly unrealistic for mining. Having lags between the decision to produce and that production turning up is key. In most areas it is easier to restart idled capacity than it is to build it fresh. Similarly some supply might be faster to "bring up" like recycling versus a new mine.
- **Single Commodity:** This is a single commodity model which is inherently limiting. Work by Isabella Weber and Bernanke /Blanchard shows that a lot of inflation shocks propagate from upstream and similarly a full production directed acyclic graph would better capture what makes sense to stockpile based upon storage cost / dependency trees / etc. 
